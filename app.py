...
# Load summarizer and tokenizer with lightweight model
    ...
    if video_url:
        ...
        try:
            ...
            if transcript:
                ...
                chunks = split_text_by_tokens(transcript, tokenizer, max_tokens=1024)
                long_summary = summarize_chunks(chunks, summarizer)

                # Check token limit
                long_summary_tokens = len(tokenizer.encode(long_summary))
                if long_summary_tokens > 1024:
                    summary_chunks = split_text_by_tokens(long_summary, tokenizer, max_tokens=1024)
