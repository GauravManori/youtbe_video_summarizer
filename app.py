    ...
    for chunk in chunks:
        if chunk.strip():
            ...
            try:
                ...
                summaries.append(chunk)  # Fallback to the original text if summarization fails
    ...
    if video_url:
        try:
            ...
            if transcript:
                ...
                if not chunks:
                    st.error("Failed to split transcript into chunks.")
                    st.stop()

                long_summary = summarize_chunks(chunks, summarizer)

                # Check token limit for final summary
                long_summary_tokens = len(tokenizer.encode(long_summary, truncation=True))
